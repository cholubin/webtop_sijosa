...
class OurimagesController < ApplicationController
  ...
  def index
    ...
    if ext == "all" and folder == "all"
      @myimages = Myimage.all(:user_id.not => current_user.id,:open_fl => true, :order => [:created_at.desc]).search_user(params[:search], params[:page])                   
    elsif ext == "all" and folder != "all"
      @myimages = Myimage.all(:user_id.not => current_user.id,:folder => Folder.get(folder).name, :open_fl => true, :order => [:created_at.desc]).search_user(params[:search], params[:page])                                   
    elsif ext != "all" and folder == "all"
      @myimages = Myimage.all(:user_id.not => current_user.id,:type => ext, :open_fl => true, :order => [:created_at.desc]).search_user(params[:search], params[:page])                                   
    elsif ext != "all" and folder != "all"
      @myimages = Myimage.all(:user_id.not => current_user.id,:folder => Folder.get(folder).name, :type => ext, :open_fl => true, :order => [:created_at.desc]).search_user(params[:search], params[:page])                           
    end
